...
Model.prototype.initialize = async function () {
  if (!this.container) {
    this.DB = await (
      await this.CLIENT.databases.createIfNotExists({ id: this.DATABASE })
    ).database;
    ``;
    this.container = await (
      await this.DB.containers.createIfNotExists({ id: this.containerName })
    ).container;
  }
};
...
Model.prototype.update = async function (id, document) {
  ...
  try {
    ...
    updatedRecord = await this.container.item(id).replace(updatedRecord);
...
//findOne - returns an object of records that contain the value of a particular key
Model.prototype.findOne = async function (key, value) {
  if (Model.hasOwnProperty(!key)) return null;
  await this.initialize();

  const { resources } = await this.container.items
    .query(`SELECT * FROM docs WHERE ${key} = "${value}"`)
    .fetchAll();
